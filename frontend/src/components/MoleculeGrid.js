...
const MoleculeGrid = ({ molecules }) => {
  ...
  // Funzione per ottenere l'immagine 2D della molecola
  const getMoleculeImageUrl = (smiles) => {
    // Utilizziamo un servizio pubblico per convertire SMILES in immagini (alternativa all'API del backend)
    return `https://cactus.nci.nih.gov/chemical/structure/${encodeURIComponent(smiles)}/image`;
  };
